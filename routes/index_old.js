...
sql[0] = 'SELECT t1.MCC, t1.MNC, t1.operator_name, t2.country_name, t2.LOC1, t2.LOC2, IF(t3.subs_count IS NULL, 0, t3.subs_count) AS subs_count_LTE, IF(t5.subs_count IS NULL, 0, t5.subs_count) AS subs_count_3G, IF(t3.subs_count IS NULL, 0, t3.subs_count)+IF(t5.subs_count IS NULL, 0, t5.subs_count) AS subs_count_Total, t4.dra_name, t6.dra_name AS dra_name_3g, t1.event  FROM country_list t2, dra_list t4, 3g_dra_list t6, operator_list t1 LEFT JOIN ob_lte_subs AS t3 ON (t1.MCC = t3.MCC AND t1.MNC = t3.MNC) LEFT JOIN ob_3g_subs AS t5 ON (t1.MCC = t5.MCC AND t1.MNC = t5.MNC) WHERE t1.MCC = t2.MCC AND t1.dra = t4.dra AND t1.3g_dra = t6.dra';
sql[1] = 'SELECT t1.MCC, t1.MNC, t1.operator_name, t2.country_name, t2.LOC1, t2.LOC2, IF(t3.subs_count IS NULL, 0, t3.subs_count) AS subs_count_LTE, IF(t5.subs_count IS NULL, 0, t5.subs_count) AS subs_count_3G, IF(t3.subs_count IS NULL, 0, t3.subs_count)+IF(t5.subs_count IS NULL, 0, t5.subs_count) AS subs_count_Total, t4.dra_name, t6.dra_name AS dra_name_3g, t1.event  FROM country_list t2, dra_list t4, 3g_dra_list t6, operator_list t1 LEFT JOIN ib_lte_subs AS t3 ON (t1.MCC = t3.MCC AND t1.MNC = t3.MNC) LEFT JOIN ib_3g_subs AS t5 ON (t1.MCC = t5.MCC AND t1.MNC = t5.MNC) WHERE t1.MCC = t2.MCC AND t1.dra = t4.dra AND t1.3g_dra = t6.dra';

var sql_in = 'SELECT t1.MCC, t1.MNC, t1.operator_name, t2.country_name, t2.LOC1, t2.LOC2, IF(t3.subs_count IS NULL, 0, t3.subs_count) AS subs_count_LTE, IF(t5.subs_count IS NULL, 0, t5.subs_count) AS subs_count_3G, IF(t3.subs_count IS NULL, 0, t3.subs_count)+IF(t5.subs_count IS NULL, 0, t5.subs_count) AS subs_count_Total, t4.dra_name, t6.dra_name AS dra_name_3g, t1.event  FROM country_list t2, dra_list t4, 3g_dra_list t6, operator_list t1 LEFT JOIN ob_lte_subs AS t3 ON (t1.MCC = t3.MCC AND t1.MNC = t3.MNC) LEFT JOIN ob_3g_subs AS t5 ON (t1.MCC = t5.MCC AND t1.MNC = t5.MNC) WHERE t1.MCC = t2.MCC AND (t1.dra_in = t4.dra OR t1.dra_in_2 = t4.dra) AND t1.3g_dra = t6.dra';
...
var sql_op_all = 'SELECT t1.MCC, t1.MNC, t1.operator_name, t2.country_name, t2.LOC1, t2.LOC2, IF(t3.subs_count IS NULL,0, t3.subs_count) AS ob_subs_count_LTE, IF(t5.subs_count IS NULL,0, t5.subs_count) AS ob_subs_count_3G, IF(t6.subs_count IS NULL,0, t6.subs_count) AS ib_subs_count_LTE, IF(t7.subs_count IS NULL,0, t7.subs_count) AS ib_subs_count_3G, t4.dra_name, t9.dra_name AS dra_name_3g, t1.HDV_available, t8.OfficeHour, t8.out_of_OfficeHour, t8.email FROM country_list AS t2, dra_list AS t4, 3g_dra_list AS t9, operator_list AS t1 LEFT JOIN ob_lte_subs AS t3 ON (t1.MCC = t3.MCC AND t1.MNC = t3.MNC) LEFT JOIN ob_3g_subs AS t5 ON (t1.MCC = t5.MCC AND t1.MNC = t5.MNC) LEFT JOIN ib_lte_subs AS t6 ON (t1.MCC = t6.MCC AND t1.MNC = t6.MNC) LEFT JOIN ib_3g_subs AS t7 ON (t1.MCC = t7.MCC AND t1.MNC = t7.MNC) LEFT JOIN contact AS t8 ON (t1.MCC = t8.MCC AND t1.MNC = t8.MNC) WHERE t1.MCC = t2.MCC AND t1.dra = t4.dra AND t1.3g_dra = t9.dra ORDER BY t3.subs_count DESC, t5.subs_count DESC, t6.subs_count DESC, t7.subs_count DESC, t1.MCC';
...
var sql_today_event_up3 = 'UPDATE operator_list t1 INNER JOIN (SELECT t1.MCC, t1.MNC FROM operator_list AS t1 LEFT JOIN ob_lte_subs AS t3 ON (t1.MCC = t3.MCC AND t1.MNC = t3.MNC) LEFT JOIN ob_3g_subs AS t5 ON (t1.MCC = t5.MCC AND t1.MNC = t5.MNC) LEFT JOIN ib_lte_subs AS t6 ON (t1.MCC = t6.MCC AND t1.MNC = t6.MNC) LEFT JOIN ib_3g_subs AS t7 ON (t1.MCC = t7.MCC AND t1.MNC = t7.MNC) WHERE (t3.subs_count IS NULL AND t5.subs_count IS NULL) OR (t6.subs_count IS NULL AND t7.subs_count IS NULL)) t2 ON (t1.MCC=t2.MCC AND t1.MNC=t2.MNC) SET EVENT=0';
var sql_temp = "(select sum(subs_count) as count from ob_lte_subs) UNION (select sum(subs_count) as count from ob_3g_subs)";
var sql_temp_ib = "(select sum(subs_count) as count from ib_lte_subs) UNION (select sum(subs_count) as count from ib_3g_subs)";
var time_offset = 0;
...
router.get('/', function(req, res, next) {
  ...
  topN = 14;
  sql_prev = sql[ob_ib_state]+ ' AND (t3.subs_count IS NOT NULL OR t5.subs_count IS NOT NULL) ORDER BY t1.event DESC, t3.subs_count+t5.subs_count DESC LIMIT '+topN;
  ...
  console.log(sql_today_event_up, condition_today_event)
        ...
        if(err){
          ...
          connection.query(sql_today_event_up2+';', function(err, rows2, fields){ //MNC없이 MCC 값만 있는 이벤트에 해당되는 국가의 사업자들 event flag에 '1'표시
            if(err){
			 ...
			 total_data = "";
			 total_data_ib = "";
		         connection.query(sql_temp+';', function(err, rows2, fields){
		         if(err){
            		 	console.log(err);
          		 }
          		 else{
				...
				for(var j=0; j<rows2.length; j++){
                			total_string += numberWithCommas(rows2[j].count);
                			if(j==0){
                       	 			total_string += "  3G: "
                		}
                
           			//total_string += numberWithCommas(rows2[0].count);
            			//total_string += "  3G: "
            			//total_string += numberWithCommas(rows2[1].count);
         		 	}
			 
         		 total_data = total_string;
			 connection.query(sql_temp_ib+';', function(err,rows_ib,fields){
			 if(err){
			 	console.log(err);
			 }
			 else{
				total_string_ib = "IB    LTE: "
				for(var j=0; j<rows_ib.length;j++){
					total_string_ib +=numberWithCommas(rows_ib[j].count);
					if(j==0){
						total_string_ib += "  3G: "
					}
        }
        
        console.log(rows_prev);
			 }
				
			 
			 total_data_ib = total_string_ib;
                         res.render('index.jade', {korea_time: korea_time, total_data: total_data,total_data_ib: total_data_ib, rows: rows, rows_all: rows_all, events : rows_eve_or_iss, issues : tmp});
...
router.get('/roaming_api/v1/card_subs', function(req, res, next){
  ...
  switch (type) {
    case '00' :
      for(var i=0; i<rows_prev.length; i++){
        ...
        if(i==0){
          korea_time = moment().tz("Asia/Seoul").format('YY-MM-DD HH:mm:ss');
          rows_prev[0].korea_time = korea_time; //i첫번째 row에만 korea_time 끼워서 전송함
	  total_string="OB    LTE: ";
          connection.query(sql_temp+';', function(err, rows2, fields){
          if(err){
            console.log(err);
	    ...
	    for(var j=0; j<rows2.length; j++){
          ...
          rows_prev[0].total_data = total_string;
        });
	console.log(rows_prev);
        }
      }
      if(moment().tz("Asia/Seoul").format('HH:mm')=='00:00'){ //매일 00시00분00초
        yy = moment().tz("Asia/Seoul").format('yyyy');
        mm = moment().tz("Asia/Seoul").format('MM');
        dd = moment().tz("Asia/Seoul").format('DD');
    ...
    case '03' :
      ...
      var sql_para = 'SELECT * FROM (' + sql[ob_ib_state] +') a1 WHERE ' + condition_string +' ORDER BY event DESC, subs_count_Total DESC';
      console.log(sql_para, condition_string)
    ...
    case '15' :
      ...
      var condition_string = "upper(a1.dra_name) LIKE upper(\'%"+val+"%\')";
      var sql_para = 'SELECT * FROM (' + sql_in +') a1 WHERE ' + condition_string +' ORDER BY event DESC, subs_count_Total DESC';
    ...
    case '09' : 
      ...
      var condition_string = "upper(a1.dra_name_3g) LIKE upper(\'%"+val+"%\')";
      var sql_para = 'SELECT * FROM (' + sql[ob_ib_state] +') a1 WHERE ' + condition_string +' ORDER BY event DESC, subs_count_Total DESC';
  ...
  case '08':
    ...
    connection.query('SELECT year, month, day, hour FROM subs_data WHERE bound='+ob_ib_state+' ORDER BY year DESC, month DESC, day DESC, hour DESC LIMIT 1;', function(err, rows0, fields){
      if(err){
        console.log(err);
      }
      else{
